...
export function StatCard({ icon, label, value, className }: StatCardProps) {
  return (
    <div className={cn("bg-white shadow-sm p-4 sm:p-5 flex flex-row items-center gap-3 sm:gap-4", className)}>
      <div className="w-10 h-10 bg-brand-primary-light flex items-center justify-center text-brand-primary shrink-0">
        {icon}
      </div>
      <div>
        <div className="text-xl sm:text-2xl font-bold text-brand-primary font-display leading-none">{value}</div>
