// @deprecated — prefer POST /api/cli/deploy with { action: "dataset.deploy", data }
//
...
// deployData handler directly so behaviour is identical to the unified
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await deployData(body);
    return NextResponse.json(result, { status: result.ok ? 200 : 400 });
  } catch (err) {
    console.error('[cli-deploy/data shim] handler error:', err);
