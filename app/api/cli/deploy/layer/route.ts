// @deprecated — prefer POST /api/cli/deploy with { action: "layer.deploy", data }
//
...
// deployLayer handler directly so behaviour is identical to the unified
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await deployLayer(body);
    return NextResponse.json(result, { status: result.ok ? 200 : 400 });
  } catch (err) {
    console.error('[cli-deploy/layer shim] handler error:', err);
