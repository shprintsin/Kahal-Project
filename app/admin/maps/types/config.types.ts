...
export interface PopupConfig {
  show: boolean;
  type: 'list' | 'template'; // 'list' shows key/value pairs, 'template' uses a string template
  fields?: string[]; // For 'list'
...
export interface GraduatedStyleConfig {
    ...
    breaks: number[]; // calculated breaks
}
...
export interface LayerConfig {
  ...
  name: string;
  type: 'polygon' | 'point';
  sourceType?: 'url' | 'database' | 'inline'; // Storage type
  data?: FeatureCollection | null; // Optional: GeoJSON data for preview/backward compat
  url?: string; // R2 URL for the GeoJSON file
  visible: boolean;
  filter?: FilterConfig;
  style: PolygonStyleConfig | PointStyleConfig;
  labels?: LabelConfig;
